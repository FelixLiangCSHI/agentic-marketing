...
if (typeof Array.prototype.at !== "function") {
  Object.defineProperty(Array.prototype, "at", {
    ...
    value(index) {
      ...
      return position >= 0 && position < this.length ? this[position] : void 0;
    }
  });
}
...
// src/analysis/metric-catalog.ts
function listSnapshotMetrics(snapshot) {
...
function metricCatalog(snapshot) {
  return new Map(
    listSnapshotMetrics(snapshot).map((metric) => [metric.metricId, metric])
  );
}
function availableMetricCatalog(snapshot) {
  return new Map(
    listSnapshotMetrics(snapshot).filter(
...
function addDays(date, days2) {
  ...
  parsed.setUTCDate(parsed.getUTCDate() + days2);
...
function dateTimePartsInZone(instant, timeZone) {
  try {
    ...
    if (!Number.isInteger(values.year) || !Number.isInteger(values.month) || !Number.isInteger(values.day) || !Number.isInteger(values.hour) || !Number.isInteger(values.minute)) {
      return null;
    }
    return {
      year: values.year,
...
function offsetMilliseconds(instant, timeZone) {
  ...
  return representedAsUtc - Math.floor(instant.valueOf() / 6e4) * 6e4;
}
function zonedDateTimeToUtc(date, time, timeZone) {
  if (!isValidIsoDate(date) || !isValidTime(time) || !isValidIanaTimeZone(timeZone)) {
    return null;
  }
  ...
  if (!represented || represented.year !== year || represented.month !== month || represented.day !== day || represented.hour !== hour || represented.minute !== minute) {
    return null;
  }
  return candidate;
}

// src/utils/stable-id.ts
...
// src/agents/action-plan-agent.ts
var MAX_POSTS_PER_WEEK = 7;
var PLAN_DURATION_DAYS = 30;
var CONTENT_CHANNELS = [
  "linkedin_page",
  "linkedin_profile"
];
var POST_TIMES = ["09:30", "14:00", "11:00", "16:30"];
...
var ActionPlanAgentError = class extends Error {
...
function inputValidationIssues(input, now) {
  const issues = [];
  ...
  for (const insight2 of input.approvedInsights) {
    if (insight2.approvalStatus !== "approved") {
      issues.push({
  ...
  for (const strategy2 of input.approvedStrategies) {
    if (strategy2.approvalStatus !== "approved") {
      issues.push({
    ...
    for (const metricId of strategy2.metricIds) {
  ...
  if (input.approvedInsights.length === 0 || input.approvedStrategies.length === 0) {
    issues.push({
...
function snapshotKpiDefinitions(input) {
...
function kpisForPlan(definitions) {
  const current = definitions.filter((definition) => definition.source === "snapshot").slice(0, 3).map((definition) => definition.metricId);
  return [
    ...current,
...
function makeRevision(changeType, summary, now) {
...
function buildSchedule(input, planId, endDate, metricIds, generatedAt) {
  var _a, _b, _c, _d;
  const calendar = [];
  const weeks = [];
  ...
  for (let weekIndex = 0; weekIndex < 4; weekIndex += 1) {
    ...
    const strategy2 = input.approvedStrategies[weekIndex % input.approvedStrategies.length];
    const contentItems = offsets.map((offset, postIndex) => {
      ...
      const experiment = isExperiment ? {
        ...
        reviewDate: addDays(
          ...
          Math.min(7, Math.max(0, Number(
            ((/* @__PURE__ */ new Date(`${endDate}T00:00:00.000Z`)).valueOf() - (/* @__PURE__ */ new Date(`${publishDate}T00:00:00.000Z`)).valueOf()) / 864e5
          )))
        ),
      ...
      const item = {
        ...
        sourceInsightIds: [...strategy2.insightIds],
    ...
    const firstPublishDate = (_b = (_a = calendar.find((item) => item.itemId === contentItems[0])) == null ? void 0 : _a.date) != null ? _b : weekStart;
    const lastPublishDate = (_d = (_c = calendar.find(
      (item) => item.itemId === contentItems[contentItems.length - 1]
    )) == null ? void 0 : _c.date) != null ? _d : weekEnd;
    const previousReviewTask = weekNumber > 1 ? [`week-${weekNumber - 1}-review`] : [];
    const tasks = [
      {
...
function isPlanItemStatus(value) {
  return value === "ai_draft" || value === "confirmed" || value === "rejected";
}
function isSocialChannel(value) {
  return value === "linkedin_page" || value === "linkedin_profile";
}
function isContentWorkflowStatus(value) {
  return [
    "planning",
    "ready_for_buffer",
    "exported_to_buffer",
...
function isContentValidationStatus(value) {
  return ["not_validated", "ready", "warning", "error"].includes(String(value));
}
function isNullableString(value) {
  return value === null || typeof value === "string";
}
function isWeekTaskShape(value) {
  return isRecord(value) && typeof value.taskId === "string" && typeof value.title === "string" && typeof value.ownerPlaceholder === "string" && typeof value.dueDate === "string" && isPlanItemStatus(value.status) && isStringArray(value.dependencies);
}
function isFourWeekPlanItemShape(value) {
  return isRecord(value) && [1, 2, 3, 4].includes(Number(value.weekNumber)) && isRecord(value.dateRange) && typeof value.dateRange.start === "string" && typeof value.dateRange.end === "string" && typeof value.objective === "string" && Array.isArray(value.tasks) && value.tasks.every(isWeekTaskShape) && isStringArray(value.contentItems) && typeof value.ownerPlaceholder === "string" && typeof value.publishDate === "string" && typeof value.targetAudience === "string" && typeof value.callToAction === "string" && isStringArray(value.kpiMetricIds) && typeof value.reviewAction === "string" && isStringArray(value.dependencies);
}
function isExperimentShape(value) {
  return isRecord(value) && typeof value.experimentId === "string" && typeof value.hypothesis === "string" && typeof value.successCriteria === "string" && typeof value.reviewDate === "string" && isStringArray(value.metricIds);
}
function isCalendarItemShape(value) {
  return isRecord(value) && typeof value.itemId === "string" && typeof value.date === "string" && typeof value.topic === "string" && typeof value.contentFormat === "string" && typeof value.targetAudience === "string" && typeof value.coreMessage === "string" && typeof value.postText === "string" && isSocialChannel(value.channel) && typeof value.scheduledTime === "string" && typeof value.timeZone === "string" && isStringArray(value.mediaUrls) && isNullableString(value.mediaRequirement) && isNullableString(value.linkUrl) && typeof value.callToAction === "string" && isNullableString(value.campaignTag) && typeof value.strategyId === "string" && isStringArray(value.sourceInsightIds) && isStringArray(value.measurementMetricIds) && isPlanItemStatus(value.status) && isContentWorkflowStatus(value.workflowStatus) && isContentValidationStatus(value.validationStatus) && isStringArray(value.validationIssues) && typeof value.isExperiment === "boolean" && (value.experiment === null || isExperimentShape(value.experiment)) && typeof value.ownerPlaceholder === "string" && typeof value.lastEditedAt === "string";
}
function isKpiDefinitionShape(value) {
  return isRecord(value) && typeof value.metricId === "string" && typeof value.label === "string" && (value.source === "snapshot" || value.source === "future_collection") && (value.availability === "available" || value.availability === "collect_next_import");
}
function isKpiReviewShape(value) {
  return isRecord(value) && typeof value.reviewId === "string" && typeof value.reviewDate === "string" && isStringArray(value.metricIds) && typeof value.action === "string" && typeof value.comparisonRule === "string";
}
function isPlanRevisionShape(value) {
  return isRecord(value) && typeof value.revisionId === "string" && typeof value.changedAt === "string" && [
    "calendar_item",
    "schedule",
    "audience",
    "plan_status",
    "buffer_handoff"
  ].includes(String(value.changeType)) && typeof value.summary === "string";
}
function isActionPlanShape(value) {
  if (!isRecord(value)) {
    return false;
  }
...
function normalizeActionPlan(value) {
  ...
  if (!isRecord(value) || value.schemaVersion !== "1.0" || value.promptVersion !== "action-plan-v1.0" || !isStringArray(value.sourceInsightIds) || typeof value.updatedAt !== "string" || !Array.isArray(value.contentCalendar)) {
    return null;
  }
  ...
  const migratedItems = value.contentCalendar.map(
    (item, index) => migrateLegacyCalendarItem(
      item,
      index,
      timeZone,
      sourceInsightIds,
...
function validateActionPlan(plan, input, now = /* @__PURE__ */ new Date()) {
  var _a, _b, _c;
  ...
  const allKpiReferences = [];
  const taskIds = new Set(
    plan.fourWeekPlan.flatMap(
      (week) => week.tasks.map((task) => task.taskId)
    )
  );
  ...
  if (plan.sourceInsightIds.length !== approvedInsightIds.size || [...approvedInsightIds].some(
  ...
  for (const item of plan.contentCalendar) {
    if (!isValidTime(item.scheduledTime) || !isValidIanaTimeZone(item.timeZone)) {
      issues.push({
        code: "INVALID_PLAN_STRUCTURE",
    ...
    item.measurementMetricIds.forEach(
      (metricId) => allKpiReferences.push({
        path: `plan.contentCalendar.${item.itemId}.measurementMetricIds`,
        metricId
      })
    );
    if (item.isExperiment && (!item.experiment || !item.experiment.hypothesis.trim() || !item.experiment.successCriteria.trim() || !isValidIsoDate(item.experiment.reviewDate) || item.experiment.reviewDate < item.date || item.experiment.reviewDate > plan.endDate || item.experiment.metricIds.length === 0)) {
      issues.push({
  ...
  for (const week of plan.fourWeekPlan) {
    ...
    if (week.contentItems.length !== plan.preferences.postsPerWeek || week.dateRange.start !== expectedWeekStart || week.dateRange.end !== addDays(expectedWeekStart, 6) || !dateWithin(week.publishDate, week.dateRange.start, week.dateRange.end)) {
      issues.push({
        code: "INVALID_PLAN_STRUCTURE",
    ...
    for (const task of week.tasks) {
      if (!dateWithin(task.dueDate, plan.startDate, plan.endDate) || task.dependencies.includes(task.taskId) || task.dependencies.some((taskId) => !taskIds.has(taskId))) {
        issues.push({
          code: "INVALID_PLAN_STRUCTURE",
  ...
  if (referencedContentItems.length !== contentItemIds.size || new Set(referencedContentItems).size !== referencedContentItems.length) {
    issues.push({
      code: "INVALID_PLAN_STRUCTURE",
  ...
  for (const [metricId, definition] of kpiDefinitions) {
    if (definition.source === "snapshot" && !availableMetrics.has(metricId)) {
      issues.push({
  ...
  const calendarByWeek = /* @__PURE__ */ new Map();
  for (const item of plan.contentCalendar) {
    const daysFromStart = ((/* @__PURE__ */ new Date(`${item.date}T00:00:00.000Z`)).valueOf() - (/* @__PURE__ */ new Date(`${plan.startDate}T00:00:00.000Z`)).valueOf()) / 864e5;
    const week = Math.floor(daysFromStart / 7) + 1;
    calendarByWeek.set(week, ((_b = calendarByWeek.get(week)) != null ? _b : 0) + 1);
  }
  for (let week = 1; week <= 4; week += 1) {
    if (((_c = calendarByWeek.get(week)) != null ? _c : 0) > plan.preferences.postsPerWeek) {
      issues.push({
        code: "INVALID_POSTS_PER_WEEK",
...
function generateActionPlan(input, now = /* @__PURE__ */ new Date()) {
  ...
  const planId = stableId(
    ...
    JSON.stringify({
      ...
      insightIds: input.approvedInsights.map((item) => item.insightId).sort(),
      strategyIds: input.approvedStrategies.map((item) => item.strategyId).sort(),
...
function reviseCalendarItem(plan, itemId, patch, now = /* @__PURE__ */ new Date()) {
...
function reviseActionPlanSchedule(plan, input, preferences, now = /* @__PURE__ */ new Date()) {
...
function confirmActionPlan(plan, now = /* @__PURE__ */ new Date()) {
  return {
    ...plan,
...
function metricCitation(metric) {
  return {
...
function metricAnswer(metric, now, possibleMeaning, suggestedValidation) {
...
function referencedMetrics(context, metricIds) {
...
function answerProjectQuestion(context, question, now = /* @__PURE__ */ new Date()) {
  var _a, _b, _c;
  ...
  if (/(data quality|quality issue|why.*unavailable|reliability)/i.test(
    ...
    const issue3 = blocking != null ? blocking : context.snapshot.quality.issues[0];
    if (!issue3) {
      return baseAnswer(now, {
    ...
    return baseAnswer(now, {
      ...
      citations: [
        {
          citationId: `quality-${issue3.code}`,
          kind: "quality",
          label: `${issue3.code} · ${issue3.module}`,
          metric: null
        }
      ],
      suggestedPlanChange: null
  ...
  if (/(insight.*evidence|why.*insight|what.*evidence)/i.test(normalized)) {
    ...
    const metrics = referencedMetrics(
      context,
      insight2.evidence.map((item) => item.metricId)
    );
    return baseAnswer(now, {
      ...
      citations: [
        {
          citationId: insight2.insightId,
          kind: "insight",
          label: insight2.title,
          metric: null
        },
  ...
  if (/(recommend|what.*publish|content direction|next month)/i.test(
    ...
    const planItems = (_c = (_b = context.plan) == null ? void 0 : _b.contentCalendar.filter((item) => item.status !== "rejected").slice(0, 2).map((item) => `${item.date}：${item.topic}`)) != null ? _c : [];
    return baseAnswer(now, {
...
// src/agents/evidence-strategy-agent.ts
function usable(metrics) {
    ...
    strategies.push(
      strategy(snapshot, {
        ...
        metricIds: audienceInsights.flatMap(
          (item) => item.evidence.map((reference) => reference.metricId)
        )
      })
    );
  }
...
function sortedUniqueDates(records) {
  return [
    ...new Set(
...
function inferGranularity(dates) {
  if (dates.length < 2) {
    return "irregular";
  }
...
function periodForRecords(records) {
...
function overlapPeriods(periods) {
  ...
  return {
    ...
    granularity: granularities.size === 1 ? available[0].granularity : "irregular",
...
function referencesForRecords(records, fields) {
  var _a;
  const groups = /* @__PURE__ */ new Map();
  for (const record of records) {
    const key = `${record.module}\0${record.source.fileName}\0${record.source.sheetName}`;
  ...
  return [...groups.values()].flatMap((group) => {
    ...
    (left, right) => left.module.localeCompare(right.module) || left.fileName.localeCompare(right.fileName) || left.sheetName.localeCompare(right.sheetName) || left.rowStart - right.rowStart
  );
}
...
function reliabilityForCoverage(availableCount, totalCount, options = {}) {
  var _a, _b;
...
function unavailableMetric(input) {
  ...
  return createMetric({
    ...input,
    value: null,
    period: (_a = input.period) != null ? _a : null,
    sourceReferences: [],
...
function fieldValue(record, field) {
  var _a;
  const value = (_a = Object.entries(record).find(([key]) => key === field)) == null ? void 0 : _a[1];
...
// src/analysis/metrics-engine.ts
var MIN_GROUP_SAMPLE = 3;
function isContentClickThroughPair(record) {
  return record.impressions !== null && record.clicks !== null;
}
function isContentEngagementPair(record) {
  return isContentClickThroughPair(record) && record.reactions !== null && record.comments !== null && record.reposts !== null;
}
function contentEngagements(record) {
  return record.clicks + record.reactions + record.comments + record.reposts;
}
function hasComparablePeriodGap(leftDate, rightDate, granularity) {
...
function withoutDuplicates(records) {
  return records.filter((record) => !record.isDuplicate);
}
function temporalFollowers(records) {
  return withoutDuplicates(records).filter((record) => record.date !== null).sort(
    (left, right) => left.date.localeCompare(right.date)
  );
}
function temporalVisitors(records) {
  return withoutDuplicates(records).filter((record) => record.date !== null).sort(
    (left, right) => left.date.localeCompare(right.date)
  );
}
function selectedContentRecords(records) {
...
function metricFromValues(input) {
  ...
  return createMetric({
    ...
    sourceReferences: input.value === null ? [] : referencesForRecords(input.records, input.fields),
...
function sumMetric(input) {
  const values = input.records.map(
    (record) => numberField(record, input.field)
  );
...
function buildSeries(records, field, seriesId, label, module2) {
...
function demographicTopN(module2, records, period, topN = 5) {
  const demographic = withoutDuplicates(records).filter(
    (record) => record.demographicDimension !== null && record.demographicValue !== null && (record.demographicCount !== null || record.demographicPercentage !== null)
  );
  const dimensions = [
    ...new Set(
      demographic.map((record) => record.demographicDimension)
    )
  ].sort();
  return dimensions.map((dimension) => {
    ...
    for (const record of dimensionRecords) {
      const key = record.demographicValue;
      const metricValue = (_a = record.demographicCount) != null ? _a : record.demographicPercentage;
      if (metricValue === null) {
        continue;
      }
      const group = (_b = groups.get(key)) != null ? _b : { value: 0, records: [] };
      group.value += metricValue;
      group.records.push(record);
      groups.set(key, group);
    }
    const sorted = [...groups.entries()].sort(
      ([leftKey, left], [rightKey, right]) => right.value - left.value || leftKey.localeCompare(rightKey)
    );
    const items = rankItems(
      ...
      dimensionRecords.some((record) => record.demographicCount !== null) ? "count" : "percentage",
...
function rankItems(entries, unit, fields) {
...
function calculateFollowersMetrics(records) {
  var _a, _b;
  ...
  const start = (_a = first == null ? void 0 : first.totalFollowers) != null ? _a : null;
...
function calculateVisitorsMetrics(records) {
  const temporal = temporalVisitors(records);
  const period = periodForRecords(temporal);
  const completePairs = temporal.filter(
    (record) => record.pageViews !== null && record.uniqueVisitors !== null
  );
  ...
  const periodChangeComparable = lastTwo.length === 2 && period !== null && period.granularity !== "irregular" && hasComparablePeriodGap(
    lastTwo[0].date,
    lastTwo[1].date,
    period.granularity
  );
  const periodChange = periodChangeComparable ? safeDivide(
    lastTwo[1].pageViews - lastTwo[0].pageViews,
    lastTwo[0].pageViews
  ) : null;
  return {
...
function contentPerformance(record) {
    ...
    return {
      ...
      metrics: [
        ...
        metricFromValues({
          ...
          label: "CTR",
          values: group.map(
            (record) => isContentClickThroughPair(record) ? record.clicks : null
          ),
...
function calculateContentMetrics(inputRecords) {
  ...
  const engagementValues = records.map(contentPerformance).filter((value) => value !== null);
  ...
  return {
    ...
    clickThroughRate: metricFromValues({
      ...
      label: "Content CTR",
      values: records.map(
        (record) => record.clicks !== null && record.impressions !== null ? record.clicks : null
      ),
    ...
    engagementRate: metricFromValues({
      ...
      values: records.map(
        (record) => isContentEngagementPair(record) ? contentEngagements(record) : null
      ),
    ...
    byWeekday: contentGroupMetrics(
      records,
      (record) => record.publishedAt ? weekdays[new Date(record.publishedAt).getUTCDay()] : null,
      "content.weekday",
      period
    )
  };
...
function calculateCrossModuleMetrics(input) {
  var _a;
  ...
  const sameGranularity = followerPeriod !== null && visitorPeriod !== null && followerPeriod.granularity !== "irregular" && followerPeriod.granularity === visitorPeriod.granularity;
  const followerByDate = new Map(
    followers.flatMap(
      (record) => record.date && record.newFollowers !== null ? [[dateKey(record.date), record]] : []
    )
  );
  const visitorByDate = new Map(
    visitors.flatMap(
      (record) => record.date && record.uniqueVisitors !== null ? [[dateKey(record.date), record]] : []
    )
  );
  const commonDates = [...followerByDate.keys()].filter((date) => visitorByDate.has(date)).sort();
  ...
  const pairCoverageValues = candidateDates.map(
    (date) => followerByDate.has(date) && visitorByDate.has(date) ? 1 : null
  );
  const trendPairs = commonDates.map((date) => ({
    date,
    follower: followerByDate.get(date),
    visitor: visitorByDate.get(date)
  }));
  const commonPeriodBase = periodForRecords(
    trendPairs.flatMap(({ follower, visitor }) => [follower, visitor])
  );
  const commonPeriod = commonPeriodBase ? { ...commonPeriodBase, sampleSize: trendPairs.length } : null;
  const trendCorrelation = sameGranularity && overlap ? pearsonCorrelation(
    trendPairs.map(({ follower, visitor }) => ({
      x: visitor.uniqueVisitors,
      y: follower.newFollowers
    }))
  ) : null;
  ...
  const proxyRatio = sameGranularity && overlap ? safeDivide(pairedNewFollowers, pairedUniqueVisitors) : null;
  const pairedAudienceRecords = trendPairs.flatMap(
    ({ follower, visitor }) => [follower, visitor]
  );
  const contentCountByDate = /* @__PURE__ */ new Map();
  for (const record of content) {
    if (!record.publishedAt) {
      continue;
    }
    const key = dateKey(record.publishedAt);
    const group = (_a = contentCountByDate.get(key)) != null ? _a : [];
    group.push(record);
    contentCountByDate.set(key, group);
  }
  const publishingPairs = commonDates.flatMap((date) => {
    var _a2;
    const posts = (_a2 = contentCountByDate.get(date)) != null ? _a2 : [];
    ...
    return [
      {
        x: posts.length,
        y: follower.newFollowers + visitor.uniqueVisitors
      }
    ];
  });
  const publishingCorrelation = sameGranularity && overlap ? pearsonCorrelation(publishingPairs) : null;
  return {
    ...
    visitorToFollowerProxyRatio: metricFromValues({
      ...
      values: candidateDates.map(
        (date) => {
          var _a2, _b;
          return followerByDate.has(date) && visitorByDate.has(date) ? (_b = (_a2 = visitorByDate.get(date)) == null ? void 0 : _a2.uniqueVisitors) != null ? _b : null : null;
        }
      ),
      value: proxyRatio,
      unit: "percentage",
    ...
    publishingWindowCorrelation: metricFromValues({
      ...
      records: [
        ...content.filter(
          (record) => record.publishedAt !== null && commonDates.includes(dateKey(record.publishedAt))
        ),
...
// src/domain/linkedin.ts
var LINKEDIN_MODULES = ["followers", "visitors", "content"];

// src/analysis/quality-engine.ts
var HIGH_NULL_RATE = 0.3;
...
function countIssueSeverities(issues, module2) {
  return {
    info: issues.filter(
      (item) => item.module === module2 && item.severity === "info"
    ).length,
    warning: issues.filter(
      (item) => item.module === module2 && item.severity === "warning"
    ).length,
    error: issues.filter(
      (item) => item.module === module2 && item.severity === "error"
    ).length
  };
}
function nullRates(records, fields) {
  if (records.length === 0) {
    return {};
  }
...
function checkMissingModules(input) {
  return LINKEDIN_MODULES.flatMap(
    (module2) => input.records[module2].length === 0 ? [
      issue({
        code: "MISSING_MODULE",
        severity: "error",
        module: module2,
...
function checkOverlapAndGranularity(periods) {
  const available = LINKEDIN_MODULES.filter(
    (module2) => periods[module2] !== null
  );
  if (available.length < 2) {
    return [];
  }
  const overlap = overlapPeriods(available.map((module2) => periods[module2]));
  const issues = [];
  if (!overlap) {
    issues.push(
      issue({
  ...
  const regularGranularities = new Set(
    available.flatMap((module2) => {
      var _a;
      const granularity = (_a = periods[module2]) == null ? void 0 : _a.granularity;
      return granularity && granularity !== "irregular" ? [granularity] : [];
    })
  );
...
function expectedGapDays(granularity) {
  if (granularity === "daily") {
...
function checkDateGaps(module2, records, period) {
  if (!period) {
    return [];
  }
...
function checkDuplicates(module2, records) {
...
function checkNullRates(module2, records) {
  if (records.length === 0) {
    return [];
  }
  return MODULE_FIELDS[module2].flatMap((field) => {
...
function checkParserIssues(module2, records) {
  const invalid = records.filter(
    (record) => record.issueReferences.some(
  ...
  const percentages = records.filter(
    (record) => record.issueReferences.some(
...
function checkFollowerDecrease(records) {
  const totals = records.filter(
    (record) => !record.isDuplicate && record.date !== null && record.totalFollowers !== null
  ).sort(
    (left, right) => left.date.localeCompare(right.date)
  );
  const affected = totals.slice(1).filter(
    (record, index) => record.totalFollowers < totals[index].totalFollowers
  );
...
function checkVisitorConsistency(records) {
  const affected = records.filter(
    (record) => record.pageViews !== null && record.uniqueVisitors !== null && record.uniqueVisitors > record.pageViews
  );
  return affected.length === 0 ? [] : [
    issue({
...
function checkContentEngagement(records) {
  const affected = records.filter((record) => {
    ...
    return components.some((value) => value > record.impressions) || components.reduce((sum, value) => sum + value, 0) > record.impressions * 2;
  });
  return affected.length === 0 ? [] : [
    issue({
...
function checkSmallSamples(input) {
  return LINKEDIN_MODULES.flatMap((module2) => {
    const records = input.records[module2];
...
function checkContentDates(content, followerPeriod, visitorPeriod) {
  const comparisonPeriod = overlapPeriods([followerPeriod, visitorPeriod]);
  if (!comparisonPeriod) {
    return [];
  }
  const affected = content.filter(
    (record) => record.publishedAt !== null && (dateKey(record.publishedAt) < comparisonPeriod.start || dateKey(record.publishedAt) > comparisonPeriod.end)
  );
  return affected.length === 0 ? [] : [
    issue({
...
function evaluateDataQuality(input) {
  ...
  const issues = [
    ...
    ...LINKEDIN_MODULES.flatMap((module2) => [
      ...checkDateGaps(module2, input.records[module2], periods[module2]),
      ...checkDuplicates(module2, input.records[module2]),
      ...checkNullRates(module2, input.records[module2]),
      ...checkParserIssues(module2, input.records[module2])
    ]),
    ...
    (left, right) => Number(right.blocksAnalysis) - Number(left.blocksAnalysis) || left.severity.localeCompare(right.severity) || left.code.localeCompare(right.code)
  );
  const moduleSummaries = Object.fromEntries(
    LINKEDIN_MODULES.map((module2) => {
      const records = input.records[module2];
      const summary = {
        module: module2,
        ...
        nullRates: nullRates(records, MODULE_FIELDS[module2]),
        period: periods[module2],
        issueCount: countIssueSeverities(issues, module2)
      };
      return [module2, summary];
    })
  );
  return {
    issues,
...
// src/analysis/snapshot-engine.ts
function snapshotIdForInput(input) {
  const signature = LINKEDIN_MODULES.flatMap(
    (module2) => input.records[module2].map((record) => {
...
function analysisInputFromParseResults(results, inputMode) {
  const allRecords = LINKEDIN_MODULES.flatMap(
    (module2) => {
      var _a, _b;
      return (_b = (_a = results[module2]) == null ? void 0 : _a.workbook.sheets.flatMap((sheet) => sheet.records)) != null ? _b : [];
    }
  );
  return {
    inputMode,
...
function generateAnalysisSnapshot(input) {
...
// src/data-processing/file-validation.ts
var MAX_UPLOAD_SIZE_BYTES = 10 * 1024 * 1024;
var MAX_REQUEST_SIZE_BYTES = MAX_UPLOAD_SIZE_BYTES + 1024 * 1024;
var SUPPORTED_FILE_EXTENSIONS = [".xlsx", ".xls", ".csv"];
var ACCEPTED_FILE_TYPES = SUPPORTED_FILE_EXTENSIONS.join(",");
var MIME_TYPES = {
  csv: [
    "text/csv",
    "application/csv",
...
var GENERIC_MIME_TYPES = ["", "application/octet-stream"];
function sanitizeUploadFileName(fileName) {
  var _a;
  const baseName = (_a = fileName.split(/[\\/]/).at(-1)) != null ? _a : "upload";
  return baseName.replace(/[\u0000-\u001F\u007F]/g, "").slice(0, 240);
}
function error(code, message, retryable = true) {
...
function looksLikeEncryptedOfficeContainer(bytes) {
  ...
  return markers.some((marker) => {
    ...
    return containsByteSequence(bytes, ascii) || containsByteSequence(bytes, utf16LittleEndian);
  });
}
function looksLikeText(bytes) {
  if (startsWith(bytes, [239, 187, 191]) || startsWith(bytes, [255, 254]) || startsWith(bytes, [254, 255])) {
    return true;
  }
  const sample = bytes.subarray(0, Math.min(bytes.length, 4096));
  return !sample.some((value) => value === 0);
}
function looksLikeSpreadsheetXml(bytes) {
  if (!looksLikeText(bytes)) {
    return false;
  }
  const sample = new TextDecoder("utf-8", { fatal: false }).decode(bytes.subarray(0, Math.min(bytes.length, 4096))).toLocaleLowerCase("en-US");
  return sample.includes("<workbook") || sample.includes("spreadsheetml");
}
function detectSpreadsheetFormat(bytes) {
  if (startsWith(bytes, [208, 207, 17, 224, 161, 177, 26, 225])) {
    return "xls";
  }
  if (startsWith(bytes, [80, 75, 3, 4])) {
    return "xlsx";
  }
...
function validateServerFile(file, bytes) {
  const envelope = validateFileEnvelope(file);
  if (!envelope.ok) {
    return envelope;
  }
  const detectedFormat = detectSpreadsheetFormat(bytes);
  if (envelope.extensionFormat === "xlsx" && detectedFormat === "xls" && looksLikeEncryptedOfficeContainer(bytes)) {
    return {
      ok: false,
      error: error(
        "ENCRYPTED_WORKBOOK",
...
// src/domain/buffer-handoff.ts
var BUFFER_CHANNEL_DEFINITIONS = {
  linkedin_page: {
    key: "linkedin_page",
    label: "LinkedIn Page",
...
var BUFFER_SUPPORTED_CHANNELS = Object.freeze(
  Object.keys(BUFFER_CHANNEL_DEFINITIONS)
);
var BUFFER_OFFICIAL_GUIDANCE = {
  reviewedAt: "2026-07-28",
  bulkUploadArticle: "https://support.buffer.com/article/926-how-to-upload-posts-in-bulk-to-buffer",
  supportedChannelsArticle: "https://support.buffer.com/article/567-supported-channels",
  queueLimitArticle: "https://support.buffer.com/article/643-how-many-posts-can-i-schedule-in-advance",
  timeZoneArticle: "https://support.buffer.com/article/514-setting-up-your-timezones-and-posting-schedules",
  freePlan: {
...
function formulaSafeCsvText(value) {
  return /^[\t\r\n ]*[=+\-@]/.test(value) ? `'${value}` : value;
}
function csvCell(value) {
  const text = value === null || value === void 0 ? "" : Array.isArray(value) ? value.join(" | ") : String(value);
...
function safeFileSlug(value, fallback) {
  const normalized = value.normalize("NFKC").trim().replace(/[<>:"/\\|?*\u0000-\u001F]/g, "-").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/[. ]+$/g, "").slice(0, 64);
  const safe = normalized || fallback;
  return /^(con|prn|aux|nul|com[1-9]|lpt[1-9])$/i.test(safe) ? `project-${safe}` : safe;
}

// src/exports/buffer-export.ts
var DIRECT_IMAGE_PATTERN = /\.(?:gif|heic|jpe?g|png|webp)$/i;
var BufferExportError = class extends Error {
  constructor(code, message, issues = []) {
    super(message);
    this.code = code;
    this.issues = issues;
...
function issue2(code, severity, contentItemId, field, message, suggestedAction, blocksExport = severity === "error") {
  return {
    code,
    severity,
    contentItemId,
...
function composeBufferPostText(item) {
...
function applyReviewValidation(plan, reviews) {
  const byId = new Map(
  ...
  return {
    ...
    contentCalendar: plan.contentCalendar.map((item) => {
      ...
      const preserveWorkflow = item.workflowStatus === "exported_to_buffer" || item.workflowStatus === "published";
      return {
        ...item,
        validationStatus: status,
        validationIssues: review.issues.filter((candidate) => candidate.severity !== "info").map((candidate) => candidate.code),
        workflowStatus: preserveWorkflow ? item.workflowStatus : item.status === "confirmed" && status !== "error" ? "ready_for_buffer" : "planning"
      };
    })
  };
}
function itemIssues(item, options, now, previouslyExportedIds) {
  const issues = [];
  ...
  if (isValidIsoDate(item.date) && (item.date < options.dateRange.start || item.date > options.dateRange.end)) {
    issues.push(
      issue2(
        "OUTSIDE_DATE_RANGE",
        "info",
  ...
  if (item.workflowStatus === "exported_to_buffer" || previouslyExportedIds.has(item.itemId)) {
    issues.push(
      issue2(
        "ALREADY_EXPORTED",
        "warning",
  ...
  if (isValidIsoDate(item.date) && isValidTime(item.scheduledTime) && isValidIanaTimeZone(item.timeZone)) {
  ...
  if (channelDefinition && composeBufferPostText(item).length > channelDefinition.maxTextLength) {
    issues.push(
      issue2(
        "TEXT_TOO_LONG",
        "error",
...
function addPairWarnings(reviews, code, keyFor, field, message, suggestedAction) {
  var _a;
  const groups = /* @__PURE__ */ new Map();
  for (const review of reviews) {
    if (!review.selected || !review.inDateRange || !review.channelIncluded) {
      continue;
    }
...
function validateBufferHandoff(plan, options, now = /* @__PURE__ */ new Date()) {
  var _a;
  const globalIssues = [];
  const validRange = isValidIsoDate(options.dateRange.start) && isValidIsoDate(options.dateRange.end) && options.dateRange.end >= options.dateRange.start;
  if (!validRange) {
    globalIssues.push(
      issue2(
        "INVALID_DATE_RANGE",
  ...
  const reviews = plan.contentCalendar.map((contentItem) => {
    const inDateRange = validRange && isValidIsoDate(contentItem.date) && contentItem.date >= options.dateRange.start && contentItem.date <= options.dateRange.end;
    ...
    return {
      ...
      channelIncluded,
      issues,
      canExport: selectedIds.has(contentItem.itemId) && inDateRange && channelIncluded && !hasBlockingIssue && !globalIssues.some((candidate) => candidate.blocksExport)
    };
  });
  addPairWarnings(
    reviews,
  ...
  for (const review of reviews.filter((candidate) => candidate.canExport)) {
    channelCounts[review.contentItem.channel] = ((_a = channelCounts[review.contentItem.channel]) != null ? _a : 0) + 1;
  }
  const mayExceedFreeQueue = Object.entries(channelCounts).some(
    ([, count]) => (count != null ? count : 0) > BUFFER_OFFICIAL_GUIDANCE.freePlan.queueCapacityPerChannel
  );
  ...
  const selectedReviews = reviews.filter(
    (review) => review.selected && review.inDateRange && review.channelIncluded
  );
  const warningCount = selectedReviews.reduce(
    (total, review) => total + review.issues.filter((candidate) => candidate.severity === "warning").length,
    0
  ) + globalIssues.filter((candidate) => candidate.severity === "warning").length;
  const blockingErrorCount = selectedReviews.reduce(
    (total, review) => total + review.issues.filter((candidate) => candidate.blocksExport).length,
    0
  ...
  return {
    ...
    reviews: reviews.map((review) => {
      ...
      return {
        ...review,
        contentItem: (_a2 = updatedPlan.contentCalendar.find(
          (item) => item.itemId === review.contentItem.itemId
        )) != null ? _a2 : review.contentItem
      };
    }),
    globalIssues,
    summary: {
...
function generateBufferChannelCsv(channel, items) {
  ...
  const rows = [
    ...
    ...items.map((item) => {
      ...
      return [
        ...
        (_a = item.mediaUrls[0]) != null ? _a : "",
        (_b = item.campaignTag) != null ? _b : "",
...
function bufferFileName(projectId, channel, options, generatedAt) {
  return [
...
function createBufferHandoffExport(plan, projectId, options, now = /* @__PURE__ */ new Date()) {
  ...
  if (preview.summary.requiresWarningAcknowledgement && !options.warningsAcknowledged) {
    const warnings = [
  ...
  for (const channel of options.channels) {
    const items = exportableReviews.filter((review) => review.contentItem.channel === channel).map((review) => review.contentItem);
    if (items.length === 0) {
      continue;
    }
    artifacts.push({
  ...
  const skippedItemIds = plan.contentCalendar.filter(
  ...
  const updatedPlan = {
    ...preview.updatedPlan,
    updatedAt: generatedAt,
    contentCalendar: preview.updatedPlan.contentCalendar.map(
      (item) => exportedSet.has(item.itemId) ? { ...item, workflowStatus: "exported_to_buffer" } : item
    ),
    revisionHistory: [
...
// src/exports/report-exports.ts
var PRIVATE_EXPORT_KEYS = /* @__PURE__ */ new Set([
  "apiKey",
  "api_key",
  "authorization",
  "buffer",
...
function isSourceReference(value) {
  return "module" in value && "fileName" in value && "sheetName" in value && "rowStart" in value && "rowEnd" in value && "fields" in value;
}
...
function sanitizeForExport(value) {
  ...
  if (typeof value === "object") {
    ...
    return Object.fromEntries(
      Object.entries(value).filter(([key]) => !PRIVATE_EXPORT_KEYS.has(key)).map(([key, child]) => [key, sanitizeForExport(child)])
    );
  }
  return void 0;
}
...
function exportFileName(projectId, type, extension, generatedAt) {
...
function generateMarkdownReport(input) {
  var _a, _b, _c;
...
function generateStructuredAnalysisJson(input, generatedAt = /* @__PURE__ */ new Date()) {
  const payload = {
    exportVersion: "1.0",
...
function createReportExportArtifacts(input, generatedAt = /* @__PURE__ */ new Date()) {
...
// node_modules/@e965/xlsx/xlsx.mjs
var XLSX = {};
XLSX.version = "0.20.3";
...
var STANDARD_FIELD_NAMES = new Set(
  Object.values(MODULE_FIELDS2).flat()
);
function normalizeHeader(value) {
  return value.replace(/^\uFEFF/, "").trim().toLocaleLowerCase("en-US").replace(/[_/]+/g, " ").replace(/\s+/g, " ");
}
function entriesForModule(module2) {
  if (module2 === "followers") {
...
function getHeaderCandidates(module2, rawHeader) {
  const normalized = normalizeHeader(rawHeader);
  return entriesForModule(module2).flatMap((entry) => {
    var _a;
    ...
    const preferred = (_a = entry.preferredAliases) == null ? void 0 : _a.map(normalizeHeader).includes(normalized);
    return [
      {
        field: entry.field,
...
function isFieldForModule(module2, field) {
  return MODULE_FIELDS2[module2].includes(field);
}
function getDemographicDimension(sheetName) {
  var _a, _b;
  ...
  return (_b = (_a = dimensions.find(([keyword]) => normalized.includes(keyword))) == null ? void 0 : _a[1]) != null ? _b : null;
}
function getMappingOverrideKey(sheetName, columnIndex, rawHeader) {
  return `${sheetName}::${columnIndex}::${normalizeHeader(rawHeader)}`;
}

// src/data-processing/module-detection.ts
var MODULE_KEYWORDS = {
  followers: ["follower", "followers", "new followers"],
  visitors: ["visitor", "visitors", "page views"],
  content: ["content", "post", "posts", "update"]
};
var GENERIC_FIELDS = /* @__PURE__ */ new Set([
  "date",
  "publishedAt",
...
function keywordScore(module2, sheetName, fileName) {
  const sheet = normalizeHeader(sheetName);
  const file = normalizeHeader(fileName);
  const sheetMatch = MODULE_KEYWORDS[module2].find(
    (keyword) => sheet.includes(keyword)
  );
  const fileMatch = MODULE_KEYWORDS[module2].find(
    (keyword) => file.includes(keyword)
  );
  const reasons = [];
  let score = 0;
  if (sheetMatch) {
...
function moduleCandidate(module2, headers, sheetName, fileName) {
  const matchedFields = /* @__PURE__ */ new Set();
  for (const header of headers) {
    for (const candidate of getHeaderCandidates(module2, header)) {
  ...
  const keyword = keywordScore(module2, sheetName, fileName);
...
function detectModule(headers, sheetName, fileName, moduleOverride) {
  const candidates = LINKEDIN_MODULES.map(
    (module2) => moduleCandidate(module2, headers, sheetName, fileName)
  ...
  return {
    ...
    requiresConfirmation: confidence2 === "low" || margin < 7,
    candidates,
    reasons: [
      ...top.reasons,
...
function locateHeaderRow(rows, sheetName, fileName, maxRows, moduleOverride) {
  let best = null;
  let bestScore = -1;
  ...
  scanRows.forEach((row, rowIndex) => {
    ...
    const topScore = (_b = (_a = detection.candidates[0]) == null ? void 0 : _a.score) != null ? _b : 0;
...
function resolveContextCandidates(module2, rawHeader, sheetName, candidates) {
  const normalizedHeader = normalizeHeader(rawHeader);
  const normalizedSheet = normalizeHeader(sheetName);
  const demographicDimension = getDemographicDimension(sheetName);
  if (module2 === "followers" && normalizedHeader === "total followers") {
  ...
  if (module2 === "visitors" && demographicDimension && (normalizedHeader === "total views" || normalizedHeader === "total visitors")) {
    return [{ field: "demographicCount", priority: 120 }];
  }
  return candidates;
}
function initialMapping(module2, sheetName, rawHeader, columnIndex, overrides) {
  ...
  if (Object.hasOwn(overrides, overrideKey)) {
    ...
    if (isFieldForModule(module2, override)) {
      return {
        priority: 200,
        mapping: {
          rawHeader,
  ...
  const candidates = resolveContextCandidates(
    ...
    rawHeader,
    sheetName,
    getHeaderCandidates(module2, rawHeader)
...
function buildFieldMappings(module2, sheetName, headers, overrides = {}) {
  var _a;
  const internal = headers.map((rawHeader, columnIndex) => ({ rawHeader, columnIndex })).filter(({ rawHeader }) => rawHeader.trim() !== "").map(
    ({ rawHeader, columnIndex }) => initialMapping(module2, sheetName, rawHeader, columnIndex, overrides)
  );
  const mappedFields = /* @__PURE__ */ new Map();
  for (const entry of internal) {
    if (entry.mapping.status !== "mapped" || !entry.mapping.standardField) {
      continue;
    }
    const entries = (_a = mappedFields.get(entry.mapping.standardField)) != null ? _a : [];
    entries.push(entry);
...
function mappedFieldSet(mappings) {
  return new Set(
    mappings.flatMap(
      (mapping) => mapping.status === "mapped" && mapping.standardField ? [mapping.standardField] : []
    )
  );
}
function requireOneOf(fields, mapped) {
  return fields.some((field) => mapped.has(field)) ? null : fields[0];
}
function getMissingCriticalFields(module2, sheetName, mappings) {
  const mapped = mappedFieldSet(mappings);
  const missing = [];
  const isDemographic = Boolean(getDemographicDimension(sheetName)) || mapped.has("demographicValue");
  if (module2 === "followers") {
    if (isDemographic) {
  ...
  if (module2 === "content") {
    ...
    if (normalizeHeader(sheetName).includes("post") && !mapped.has("title") && !mapped.has("contentId")) {
      missing.push("title");
    }
    const metric = requireOneOf(
      [
...
// src/data-processing/normalizers.ts
var EMPTY_MARKERS = /* @__PURE__ */ new Set([
  "",
  "-",
  "--",
  "n/a",
...
function normalizeNumericString(value) {
  ...
  if (unsigned.includes(",") && unsigned.includes(".")) {
    normalized = unsigned.lastIndexOf(",") > unsigned.lastIndexOf(".") ? unsigned.replace(/\./g, "").replace(",", ".") : unsigned.replace(/,/g, "");
  } else if (unsigned.includes(",")) {
    const commaParts = unsigned.split(",");
    normalized = commaParts.length === 2 && commaParts[1].length <= 2 ? unsigned.replace(",", ".") : unsigned.replace(/,/g, "");
  }
...
function normalizeNumber(raw) {
  ...
  const normalized = typeof raw === "number" ? raw : Number(normalizeNumericString(rawToString(raw)));
...
function normalizePercentage(raw, cellFormat) {
  if (raw === null) {
    return { value: null, problems: [] };
  }
  const stringValue2 = rawToString(raw);
  const hasPercentSign = stringValue2.endsWith("%");
  const numeric = normalizeNumber(
    hasPercentSign ? stringValue2.slice(0, -1) : raw
  );
  if (numeric.value === null) {
    return numeric;
  }
  const isExcelPercentage = typeof raw === "number" && Boolean(cellFormat == null ? void 0 : cellFormat.includes("%"));
...
function parseDateString(value) {
  ...
  const namedMonth = value.replace(/,/g, "").match(/^([a-z]+)\s+(\d{1,2})\s+(\d{4})$/i);
  if (namedMonth) {
    const month = MONTHS[namedMonth[1].toLocaleLowerCase("en-US")];
    return {
      date: month === void 0 ? null : validUtcDate(Number(namedMonth[3]), month, Number(namedMonth[2])),
      ambiguous: false
    };
  }
  ...
  if (dayNamedMonth) {
    ...
    return {
      date: month === void 0 ? null : validUtcDate(
...
function normalizeDate(raw, field) {
  if (raw === null) {
    return { value: null, problems: [] };
  }
  let parsed = null;
  ...
  if (typeof raw === "number" && Number.isFinite(raw)) {
    ...
    parsed = new Date(excelEpoch + raw * 864e5);
  ...
  return {
    value: field === "publishedAt" ? parsed.toISOString() : parsed.toISOString().slice(0, 10),
    problems
  };
}

// src/server/parsing/spreadsheet-parser.ts
var MAX_SHEETS_PER_WORKBOOK = 30;
var MAX_ROWS_PER_SHEET = 5e4;
var MAX_HEADER_SCAN_ROWS = 40;
var PREVIEW_ROW_COUNT = 5;
var SpreadsheetParseException = class extends Error {
  constructor(parseError2) {
    super(parseError2.message);
    this.name = "SpreadsheetParseException";
    this.parseError = parseError2;
  }
};
function parseError(code, message, retryable = true) {
  return new SpreadsheetParseException({ code, message, retryable });
}
function getSafeParseError(reason) {
  if (reason instanceof SpreadsheetParseException) {
    return reason.parseError;
  }
  const technicalMessage = reason instanceof Error ? reason.message.toLocaleLowerCase("en-US") : "";
  ...
  if (/corrupt|invalid|unsupported|bad (zip|file)|cannot find/.test(
    technicalMessage
  )) {
    return {
      code: "CORRUPT_FILE",
...
function emptyDetection(reason) {
  return {
    detectedModule: null,
    confidence: "low",
    requiresConfirmation: true,
...
function emptySheetResult(sheetName, code, message) {
  const issue3 = {
    code,
...
function readCell(sheet, absoluteRowIndex, columnIndex, fallback) {
  var _a;
  ...
  if (cell == null ? void 0 : cell.f) {
    return {
      rawValue: null,
      numberFormat: cell.z === void 0 || cell.z === null ? void 0 : String(cell.z),
      isFormula: true
    };
  }
  return {
    rawValue: toSerializableRawValue((_a = cell == null ? void 0 : cell.v) != null ? _a : fallback),
    numberFormat: (cell == null ? void 0 : cell.z) === void 0 || cell.z === null ? void 0 : String(cell.z),
    isFormula: false
  };
}
...
var PERCENTAGE_FIELDS = /* @__PURE__ */ new Set([
  "demographicPercentage",
  "engagementRate",
  "clickThroughRate"
]);
function normalizeCell(field, rawValue, numberFormat) {
...
function issueFromProblem(problem, source, field, rawValue) {
  return {
    code: problem.code,
...
function stringValue(values, field) {
  const value = values[field];
  return typeof value === "string" ? value : null;
}
function numberValue(values, field) {
  const value = values[field];
  return typeof value === "number" ? value : null;
}
function recordIssueReferences(issues) {
  return issues.map(({ code, field }) => ({ code, field }));
}
function createNormalizedRecord(module2, source, values, rawValues, issues) {
  const common = {
    module: module2,
    source,
    rawValues,
...
function hasRequiredRecordValues(record, sheetName) {
  ...
  if (record.module === "followers") {
    if (demographicSheet || record.demographicValue) {
      return Boolean(record.demographicValue) && (record.demographicCount !== null || record.demographicPercentage !== null);
    }
    return Boolean(record.date) && hasAnyNumber([
  ...
  const hasIdentity = !sheetName.toLocaleLowerCase("en-US").includes("post") || Boolean(record.contentId || record.title);
  return hasIdentity && Boolean(record.publishedAt) && hasAnyNumber([
    record.impressions,
...
function parseRow(module2, fileName, sheetName, sheet, row, absoluteRowIndex, mappings) {
  const source = {
    module: module2,
    fileName,
    sheetName,
  ...
  for (const mapping of mappings) {
    ...
    issues.push(
      ...normalized.problems.map(
        (problem) => issueFromProblem(problem, source, field, cell.rawValue)
      )
    );
  }
  const demographicDimension = getDemographicDimension(sheetName);
  ...
  const hasErrors = issues.some((issue3) => issue3.severity === "error");
  return {
    record,
    issues,
...
function markDuplicates(rows, sheetName) {
  ...
  rows.forEach(({ record }, index) => {
    ...
    issues.push(issue3);
    record.issueReferences.push({ code: issue3.code });
  });
...
function hasMoreRowsThanLimit(sheet) {
  const fullReference = sheet["!fullref"];
  const reference = typeof fullReference === "string" ? fullReference : sheet["!ref"];
  if (typeof reference !== "string") {
    return false;
  }
  const range = utils.decode_range(reference);
...
function parseSheet(sheetName, sheet, input) {
  ...
  if (!detectedModule) {
    const dataRows = rows.slice(located.rowIndex + 1).filter((row) => row.some((value) => !isBlankValue(value)));
    return {
      sheetName,
  ...
  for (let relativeRowIndex = firstDataRowIndex; relativeRowIndex < lastDataRowIndex; relativeRowIndex += 1) {
  ...
  const hasStructuralErrors = issues.some(
    (issue3) => issue3.severity === "error" && (issue3.scope === "sheet" || issue3.rowNumber === void 0)
  );
  ...
  return {
    ...
    issues,
    canProceed: validRows > 0 && !hasStructuralErrors && !detection.requiresConfirmation
  };
}
function parseSpreadsheetBytes(input) {
  ...
  const sheets = workbook.SheetNames.map((sheetName) => {
    ...
    return sheet ? parseSheet(sheetName, sheet, {
  ...
  const detectedModules = [
    ...new Set(
      sheets.flatMap(
        ({ detection }) => detection.detectedModule ? [detection.detectedModule] : []
      )
    )
  ];
...
// src/server/parsing/synthetic-results.ts
function createSyntheticParseResults() {
  const encoder = new TextEncoder();
  const results = LINKEDIN_MODULES.map((module2) => {
    const definition = SYNTHETIC_FILES[module2];
...
// src/streamlit/bridge.ts
var BRIDGE_PROTOCOL_VERSION = "1.0";
var MAX_BRIDGE_REQUEST_BYTES = 43 * 1024 * 1024;
var BridgeOperationError = class extends Error {
  constructor(details) {
...
function isLinkedInModule(value) {
  return typeof value === "string" && LINKEDIN_MODULES.includes(value);
}
function parseRequest(value) {
  ...
  return {
    requestId,
    operation,
    payload: isRecord2(request.payload) ? request.payload : {}
  };
}
function parseUploadedFile(value) {
  ...
  return {
    ...
    mimeType: typeof file.mimeType === "string" ? file.mimeType.slice(0, 200) : "",
    size,
    base64: requiredString(file.base64, "file.base64", 141e5)
  };
}
...
function issueSummary(issue3) {
  return {
    code: issue3.code,
    severity: issue3.severity,
    scope: issue3.scope,
    message: issue3.message,
    sheetName: issue3.sheetName,
    rowNumber: issue3.rowNumber,
    field: issue3.field
  };
}
function previewRecord(record) {
  const fields = Object.fromEntries(
    Object.entries(record).filter(
      ([key]) => key !== "rawValues" && key !== "issueReferences" && key !== "source"
    )
  );
  return {
    ...fields,
...
function summarizeParseResult(slot, result) {
  return {
    slot,
    file: result.file,
...
function emptyStrategyBundle(snapshot, generatedAt) {
  return {
    promptVersion: "evidence-strategy-v1.0",
    snapshotId: snapshot.snapshotId,
    generatedAt: generatedAt.toISOString(),
...
function buildAnalysisProject(results, inputMode, now) {
  const snapshot = generateAnalysisSnapshot(
    analysisInputFromParseResults(results, inputMode)
  );
  const strategyBundle = snapshot.canEnterInsights ? generateEvidenceStrategyBundle(snapshot, now) : emptyStrategyBundle(snapshot, now);
  return {
    mode: inputMode,
...
function parseUploadedResults(filesValue) {
  ...
  const results = {};
  const detectedOwners = /* @__PURE__ */ new Map();
  for (const file of files) {
    ...
    let bytes = decodeBase64(file.base64, file.size);
    try {
      ...
      const error2 = getSafeParseError(reason);
      throw operationError(error2.code, error2.message, error2);
    } finally {
      bytes == null ? void 0 : bytes.fill(0);
      bytes = null;
    }
  }
  return results;
...
function snapshotFromPayload(value) {
...
function planFromPayload(value, allowNull = false) {
...
function stringArray(value, fieldName, maxItems, maxItemLength) {
...
function isBufferChannel(value) {
  return BUFFER_SUPPORTED_CHANNELS.includes(value);
}
function channelArray(value, fieldName) {
...
function dateRangeFromPayload(value, fieldName) {
...
function codeFromUnknown(reason) {
  if (!isRecord2(reason)) {
    return void 0;
  }
  return typeof reason.code === "string" || typeof reason.code === "number" ? reason.code : typeof reason.status === "number" ? reason.status : void 0;
}
function bridgeErrorFromUnknown(reason) {
  if (reason instanceof BridgeOperationError) {
    return reason.details;
  }
  if (reason instanceof Error && reason.name === "ActionPlanAgentError" && "code" in reason && "issues" in reason) {
    return actionPlanError(reason).details;
  }
...
function approvedPlanInput(snapshot, strategyBundle, businessGoal, preferences) {
  return {
    snapshot,
    businessGoal,
    approvedInsights: strategyBundle.insights.filter(
      (insight2) => insight2.approvalStatus === "approved"
    ),
    approvedStrategies: strategyBundle.strategies.filter(
      (strategy2) => strategy2.approvalStatus === "approved"
    ),
    preferences
  };
}
async function runOperation(request) {
  ...
  if (request.operation === "answer_question") {
    ...
    const question = requiredString(request.payload.question, "question", 2e3);
  ...
  if (request.operation === "revise_calendar_item") {
    ...
    if ("linkUrl" in patchValue) {
      patch.linkUrl = nullableString(patchValue.linkUrl, "patch.linkUrl", 2e3);
    }
...
async function handleBridgeRequest(value) {
